...
from ..models.indexer_uploads import IndexerUploads, DataKindEnum
...
class DisputeResolver(object):
    ...
    async def get_stage(self):
        current_stage = (
            await Dispute.select("dispute_stage")
    ...
    async def indexer_add_poi(self, content, file_name):
        ...
        push_to_db = await IndexerUploads.create(
            ...
            data_kind_enum=DataKindEnum.poi,
        )
        return object_path
    ...
    async def generate_divergent_blocks(self) -> Optional[pd.DataFrame]:
        ...
        if stage not in [DisputeStage.generated_divergent_blocks, DisputeStage.acquired_poi]:
            return None
        ...
        async with db.transaction(isolation="serializable") as tx_root:
            ...
            try:
                ...
                dispute = await dispute.update(
                    dispute_stage=DisputeStage.generated_divergent_blocks
                ).apply()
    ...
    async def indexer_add_entities(self, content, file_name):
        ...
        if state != DisputeStage.generated_divergent_blocks:
            # The dispute is inactive. Can't do anything.
        ...
        push_to_db = await IndexerUploads.create(
            ...
            indexer_id=self.indexer_id,
            data_path=object_path,
            data_kind_enum=DataKindEnum.entities,
        )
        return object_path

    async def indexer_add_metdata(self, content, file_name):
        """Keep configuration and environment variables persisted to determine root causes"""
        is_implicated = await self.indexer_in_dispute()
        ...
        push_to_db = await IndexerUploads.create(
            ...
            indexer_id=self.indexer_id,
            data_path=object_path,
            data_kind_enum=DataKindEnum.metadata,
        )
        return object_path
