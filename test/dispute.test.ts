...
import { createIndexingDisputeCommand, createQueryDisputeCommand } from '../src/commands/create'
...
describe('Dispute Creation Commands', () => {
  ...
  it('should successfully create indexing dispute on happy path', async () => {
    ...
    const mockCreateIndexingDispute = sinon.stub().resolves({ hash: '0x123abc' })
    ...
    expect(mockCreateIndexingDispute.calledWith(
      '0xallocation123', 
      '0xpoi123', 
      12345
    )).to.be.true
  ...
  it('should successfully create query dispute on happy path', async () => {
    ...
    const requestCID = ethers.keccak256(ethers.toUtf8Bytes('test-query-request'))
    const responseCID = ethers.keccak256(ethers.toUtf8Bytes('test-query-response'))
    const subgraphDeploymentId = ethers.keccak256(ethers.toUtf8Bytes('test-subgraph-deployment'))
    ...
    expect(askConfirmStub.called).to.be.true
    expect(approveStub.called).to.be.true
    expect(mockCreateQueryDispute.calledWith(ethers.hexlify(attestationData))).to.be.true
    expect(waitTxStub.called).to.be.true
  })
